<?php
/**
 * Branch: Incoming documents (forwarded to me, awaiting receipt) + RECEIVE action.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['receive'])) {
    ...
    if ($doc && (int)$doc['current_holder_id'] === $uid && awaiting_receipt($id, $uid)) {
        ...
        header('Location: ' . BASE_URL . '/branch/incoming.php');
        exit;
    }
    header('Location: ' . BASE_URL . '/branch/incoming.php');
    exit;
...
<tr>
    ...
    <td class="small text-muted"><?= e(date('M d, Y g:iA', strtotime($r['received_at']))) ?></td>
    <td class="small">
