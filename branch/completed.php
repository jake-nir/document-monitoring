<?php
/**
 * Branch: Completed documents (APPROVED / SIGNED final state) for this branch.
 */
...
$stmt = $pdo->prepare("SELECT d.*, b.branch_name AS origin_branch, cu.full_name AS holder_name FROM documents d
    ...
    WHERE d.originating_branch_id=? AND d.current_status IN ('APPROVED','SIGNED') ORDER BY d.updated_at DESC LIMIT 50");
$stmt->execute([$branchId]);
$rows = $stmt->fetchAll();

page_header('Completed Documents', 'completed', $user);
?>
<h4 class="mb-3"><i class="bi bi-check2-circle me-2"></i>Completed Documents</h4>
<div class="card shadow-sm border-0"><div class="card-body p-0">
<table class="table table-hover table-sm align-middle mb-0">
<thead class="table-light small"><tr><th>Tracking No.</th><th>Subject</th><th>Status</th><th>Updated</th><th></th></tr></thead>
<tbody>
...
<?php endforeach; ?>
<?php if (!$rows): ?><tr><td colspan="5" class="text-center text-muted small py-4">No completed documents.</td></tr><?php endif; ?>
</tbody></table>
</div></div>
<?php page_footer(); ?>
